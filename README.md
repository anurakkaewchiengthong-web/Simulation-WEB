...
## DEM stored in GitHub

The default study area centred at **16.029156° N, 100.627809° E** with a 25 km radius uses [`dem/study-area-z12.png`](dem/study-area-z12.png). This is a lossless 1536 × 1536 Terrarium RGB mosaic of 36 Mapzen terrain tiles at Web Mercator zoom 12 (approximately 37 m between output pixels near this latitude). It is under 1 MB rather than storing a multi-gigabyte source dataset. [`dem/study-area.json`](dem/study-area.json) contains its georeferencing, source, and attribution. The underlying elevation source may have lower native resolution; this file does not add elevation detail beyond its source.

When the complete study square fits inside this mosaic, the browser reads the local DEM and needs no AWS terrain request. Moving the station or increasing the radius beyond it uses the previous AWS terrain tile download; a missing or unreadable local file also falls back to AWS. The DEM affects the **FSPL + clutter + DEM** mode only. Hata and COST‑231 Hata remain statistical models that do not apply the DEM to their path loss. The satellite basemap is independent from DEM.

Mapzen Terrain Tiles were accessed on 2026-09-24 from [the AWS Open Data registry](https://registry.opendata.aws/terrain-tiles/). See [source-specific terrain data credits](https://github.com/tilezen/joerd/blob/master/docs/attribution.md) and [Mapzen data rights](https://www.mapzen.com/rights/). This terrain file is **not** the user's separate 3 GB DEM, which would require their original data to clip and redistribute.
...
1. For the default study area, read the lossless Mapzen Terrarium mosaic bundled with the site at zoom 12; elsewhere read Mapzen Terrarium terrain tiles (AWS Open Data) at an adaptive zoom. Decode terrain metres as `R × 256 + G + B / 256 − 32768`.
